...
{
    public partial class tipo_proveedor : MetroForm
    {
        int est;
        ...
        private void tipo_proveedor_Load(object sender, EventArgs e)
        {
            ...
            {
                string cmdd = "select max (cod_tsup+1) as Mayor from tipo_sup";
        ...
        public void mostrar()
        {
            ...
            string cmd = "select * from tipo_sup";
        ...
        private void salvar_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    string cmd = "exec act_tip_sup '" + codigo.Text + "','" + descripcion.Text + "','" + fecha.Text + "','" + est + "'";
        ...
        private void eliminar_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("SEGURO QUE DESEAS ELIMINAR EL REGISTRO ACTUAL? ", " ALMACEN ", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                string cmd = "delete from tipo_sup where cod_tsup='" + codigo.Text.Trim() + "'";
                utilidades.UTILIDADES.ejecutar(cmd);
        ...
        private void actualizar_Click_1(object sender, EventArgs e)
        {
            ...
            {
                string cmd = "update tipo_sup set cod_tsup='" + codigo.Text.Trim() + "', " + "descripcion='" + descripcion.Text.Trim() + "', " + "fecha_reg='" + fecha.Value.Date.ToString("dd/MM/yyyy") + "', " + "cod_estado='" + est + "' where cod_tsup ='" + codigo.Text.Trim() + "'";
                utilidades.UTILIDADES.ejecutar(cmd);
        ...
        private void consultar_Click_1(object sender, EventArgs e)
        {
            ...
            if (cod.Checked)
            {
                ...
                {
                    string cmd = "select * from tipo_sup";
                    cmd += " where cod_tsup like('%" + busca.Text.Trim() + "%')";
        ...
        private void codigo_Validating_1(object sender, CancelEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(codigo.Text.Trim()))
            {
                cmd = "select max(cod_tsup)as mayor from tipo_sup";
                ds = utilidades.UTILIDADES.ejecutar(cmd);
            ...
            cmd = "select * from tipo_sup where cod_tsup='" + codigo.Text.Trim() + "'";
            ds = utilidades.UTILIDADES.ejecutar(cmd);
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
