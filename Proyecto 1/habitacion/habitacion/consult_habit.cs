...
{
    public partial class consult_habit : Form
    {
        public consult_habit()
        {
            InitializeComponent();
        }
        ...
        private void consult_habit_Load(object sender, EventArgs e)
        {
            DataSet ds = new DataSet();
            string cmd = "select * from habitacion";
        ...
        private void buscar_Click(object sender, EventArgs e)
        {
            if (nombre.Checked)
            {
                ...
                {
                    MessageBox.Show("NO HAY HABITACION PARA CONSULTAR");
                }
                if (string.IsNullOrEmpty(consultar.Text.Trim()) == false)
                {
                    string cmd = "select * from habitacion";
                    cmd += " where descriphab like ('%" + consultar.Text.Trim() + "%')";
                ...
                if (codigo.Checked)
                {
                    ...
                    {
                        string cmd = "select * from habitacion";
                        cmd += " where codhab like('%" + consultar.Text.Trim() + "%')";
                        DataSet ds = utilidades.UTILIDADES.ejecutar(cmd);
                        dataGridView1.DataSource = ds.Tables[0];
                    }
                    consultar.Clear();
