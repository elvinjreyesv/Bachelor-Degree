...
{
    public partial class consult_temat : Form
    {
        public consult_temat()
        {
            InitializeComponent();
        }

        private void consult_temat_Load(object sender, EventArgs e)
        {
            DataSet ds = new DataSet();
            string cmd = "select * from tematica";
        ...
        private void buscar_Click(object sender, EventArgs e)
        {
            if (nombre.Checked)
            {
                ...
                {
                    MessageBox.Show("NO HAY TEMATICA  PARA CONSULTAR");
                }
                if (string.IsNullOrEmpty(consultar.Text.Trim()) == false)
                {
                    string cmd = "select * from tematica";
                    cmd += " where descriptem like ('%" + consultar.Text.Trim() + "%')";
                ...
                if (codigo.Checked)
                {
                    ...
                    {
                        string cmd = "select * from tematica";
                        cmd += " where codtem like('%" + consultar.Text.Trim() + "%')";
                        DataSet ds = utilidades.UTILIDADES.ejecutar(cmd);
                        dataGridView1.DataSource = ds.Tables[0];
                    }
                    consultar.Clear();
