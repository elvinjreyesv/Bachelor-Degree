...
{
    public partial class consult_tipoemp : Form
    {
        public consult_tipoemp()
        {
            InitializeComponent();
        }

        private void consult_tipoemp_Load(object sender, EventArgs e)
        {
            DataSet ds = new DataSet();
            string cmd = "select * from tipoemp";
        ...
        private void buscar_Click(object sender, EventArgs e)
        {
            if (nombre.Checked)
            {
                ...
                {
                    MessageBox.Show("NO HAY TIPO DE EMPLEADO PARA CONSULTAR");
                }
                ...
                if (codigo.Checked)
                {
                    ...
                    if (string.IsNullOrEmpty(consultar.Text.Trim()))
                    {
                        MessageBox.Show("NO HAY TIPO DE EMPLEAOD  PARA CONSULTAR");
                    }
                    if (string.IsNullOrEmpty(consultar.Text.Trim()) == false)
                    {
                        string cmd = "select * from tipoemp";
