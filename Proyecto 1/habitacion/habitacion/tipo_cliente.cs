...
{
    public partial class tipo_cliente : Form
    {
        public tipo_cliente()
        {
            InitializeComponent();
            string cmdd = "select max (codtipo+1) as Mayor from tipocliente";
        ...
        private void codtipo_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                cmd = "select max(codtipo)as mayor from tipocliente";
            ...
            cmd = "select * from tipocliente where codtipo=" + codtipo.Text.Trim();
            ds = utilidades.UTILIDADES.ejecutar(cmd);
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
        ...
        private void salvar_Validating(object sender, CancelEventArgs e)
        {
            if (string.IsNullOrEmpty(codtipo.Text.Trim()))
            {
            ...
            else
            {
                try
                {
                    string cmd = "exec actualizartipcliente " + codtipo.Text + ",'" + descripcion.Text + ",'" + System.DateTime.Now + "'";
        ...
        {
            if (MessageBox.Show("DESEA ELIMINAR EL CAMPO ACTUAL? ", " TIPO DE CLIENTES ", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                int c = Convert.ToInt16(codtipo.Text);
                string cmd = "delete from tipocliente where codtipo=" + codtipo.Text.Trim();
                utilidades.UTILIDADES.ejecutar(cmd);
        ...
        private void salir_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("REALMENTE DESEA SALIR? ", " TIPO DE CLIENTES ", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                this.Hide();
            }
        }
        ...
        private void tipo_cliente_Load(object sender, EventArgs e)
        {
            descripcion.Select();
        }
        ...
        {
            consult_tipcliente c = new consult_tipcliente();
            c.ShowDialog();
            c.Dispose();
        }
