...
namespace Proyecto_3.cxc2.procesos
{
    public partial class notadc : MetroForm
    {
        ...
        private void insertar_data()
        {
            ...
            comando.CommandText = "SELECT CONVERT (char(10), fecreg, 103) as fecreg,CAST ( numreg AS varchar (50)) as numreg,floor(round(balreg,0)) as balreg,floor(round(totreg,0)) as totreg,codcli,CONVERT (char(10), fecven, 103) as fecven from cxcdbfregistros where balreg>0 and codcli='" + codcli.Text + "' order by fecreg";
        ...
        {
            string cmdd = "select MAX(numreg)+1 as mayor from cxcdbfdebcre";
      ...
      //numreg,numfac,codcli,refreg,totitb,totreg,ncfreg,ncffac,balcli,estado
                   //----------------------------------detalle del cobro-------------------
                       for (f = 0; f <= data.Rows.Count - 1; f++)
                       {
                           if (Convert.ToInt16( data.Rows[f].Cells[7].Value) ==1)
                           {
                              ...
                              string t = "select ncf from factura where numfac='" + data.Rows[f].Cells[0].Value.ToString() + "'";
                              DataSet ds5= utilidades.UTILIDADES.ejecutar(t);
                              string ncffac = Convert.ToString(ds5.Tables[0].Rows[0]["ncf"]); ;

                              string cmd = "act_notacxc '" + cobro.Text + "','" + Convert.ToInt16(data.Rows[f].Cells[0].Value) + "','" + codcli.Text + "','" + refcob.Text + "','" + itcob.Text + "','" + Convert.ToInt16(data.Rows[f].Cells[5].Value) + "','" + ncf.Text + "','" + ncffac + "','" + Convert.ToInt16(data.Rows[f].Cells[4].Value) + "','" + 1 + "','" + tip + "','" + feccob.Text +"'";
               ...
               string ncf2 = "update ncf set ultimo='" + num1 + "' where numero='" + notac + "'";
        ...
        private void notadc_Load(object sender, EventArgs e)
        {
            codigo_mayor();
            nd.Checked=true;
        }
        ...
        {
            query = " select * from cobros where numcob='" + cob + "'";
        ...
        private void data_EditingControlShowing_1(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
        ...
        private void salvar_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                dtcompra datos = GenerarFactura();
                res_notadebcre frm = new res_notadebcre(datos);
                frm.Show();
        ...
        private void condicion_rep()
        {
            ...
            query = "SELECT pe.nombre + ' ' + pe.apellido AS nombre,pe.celular,pe.direccion,cxc.codcli,cxc.numreg,cxc.fecreg,cxc.numfac,cxc.codcli,cxc.refreg,cxc.totreg,cxc.ncfreg,cxc.ncffac,cxc.balcli,cxc.tipreg,pe.telefono,pe.direccion,cxc.totitb  from cxcdbfdebcre as cxc INNER JOIN dbo.cliente AS cli ON cli.cod_cli = cxc.codcli INNER JOIN dbo.persona AS pe ON cli.cod_persona = pe.cod_persona where numreg='"+reg+"'";
            ds = utilidades.UTILIDADES.ejecutar(query);
            datos.DataSource = ds.Tables[0];
           int f = datos.Rows.Count - 1;
        }
        ...
        private dtcompra GenerarFactura()
        {
            ...
            {
       dtcompra.DatosNotacxcRow rowDatosCliente = facturacion.DatosNotacxc.NewDatosNotacxcRow();
                rowDatosCliente.numreg = Convert.ToInt16(row.Cells["numreg"].Value);
                rowDatosCliente.numfac = Convert.ToInt16(row.Cells["numfac"].Value);
                rowDatosCliente.codcli = Convert.ToString(row.Cells["codcli"].Value);
                rowDatosCliente.refreg = Convert.ToString(row.Cells["refreg"].Value);
                ...
                rowDatosCliente.cliente = Convert.ToString(row.Cells["nombre"].Value);
                ...
                rowDatosCliente.telefono = Convert.ToString(row.Cells["telefono"].Value);
          rowDatosCliente.itbis = Convert.ToDouble(row.Cells["totitb"].Value);
                facturacion.DatosNotacxc.AddDatosNotacxcRow(rowDatosCliente);
            }
            return facturacion;
        }
